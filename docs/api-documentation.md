...
**Условия:**
- Завершить можно **только заказ в статусе «Оплачен» (paid)**. В статусе «Готов к оплате» завершение недоступно (клиент мог не оплатить).
- Перед списанием проверяется наличие всех запчастей заказа на складе. Если какой-то позиции не хватает — завершение невозможно, возвращается ошибка с перечнем недостающих позиций.
- Списываются только запчасти из каталога (с `part_id`); ручные строки без привязки к запчасти не списываются.

**Особенности:**
- При завершении создаются транзакции расхода по каждой запчасти заказа и уменьшается остаток на складе
...
**Возможные ошибки:**
- `400 Bad Request` - Заказ-наряд уже завершен
- `400 Bad Request` - Завершить можно только после полной оплаты (статус не «Оплачен»)
- `400 Bad Request` - Нельзя завершить заказ: недостаточно запчастей на складе (в теле перечень позиций)
- `404 Not Found` - Заказ-наряд не найден
- `400 Bad Request` - У пользователя не привязан сотрудник
...
### GET `/api/v1/parts/`
**Описание:** Получение списка запчастей. Поддерживается поиск **строго по артикулу** (part_number), без учёта регистра. Артикул — уникальная сущность в каталоге; наименование может меняться у поставщиков, поэтому при оприходовании поиск ведётся только по артикулу.
...
- `skip` (integer, default: 0) - Количество записей для пропуска
- `limit` (integer, default: 100) - Максимальное количество записей
- `search` (string, optional) - Поиск по артикулу (part_number), подстрока
...
**Описание:** Журнал движений склада (приход, расход, корректировки)

**Права доступа:** Требуется авторизация (любая роль)

**Параметры запроса (Query):**
- `date_from` (date, optional) - Дата с (YYYY-MM-DD)
- `date_to` (date, optional) - Дата по (включительно)
- `transaction_type` (string, optional) - Тип: `incoming`, `outgoing`, `adjustment`
- `part_id` (integer, optional) - Фильтр по запчасти
- `order_id` (integer, optional) - Фильтр по заказ-наряду
- `receipt_id` (integer, optional) - Фильтр по накладной
- `skip` (integer, default: 0)
- `limit` (integer, default: 100)

**Ответ:** Массив объектов с полями: `id`, `warehouse_item_id`, `transaction_type`, `quantity`, `price`, `order_id`, `receipt_id`, `employee_id`, `created_at`, а также вложенные `part`, `order_number`, `receipt_number`, `employee_name`.

---

### POST `/api/v1/warehouse/transactions/adjustment`
**Описание:** Ручная корректировка остатка по позиции склада
...
```
- `quantity_delta` (decimal, required) - Изменение: положительное — увеличение, отрицательное — уменьшение
- `reason` (string, optional) - Причина корректировки

**Ответ:** Объект транзакции типа `adjustment`.

**Возможные ошибки:**
- `400 Bad Request` - Остаток не может стать отрицательным
- `404 Not Found` - Позиция склада не найдена
- `400 Bad Request` - У пользователя не привязан сотрудник

---

### GET `/api/v1/warehouse/reports/supplier-receipts`
**Описание:** Отчёт по приходным накладным поставщика за период (сверка по поставщику). Период задаётся по дате документа накладной (`document_date`).

**Права доступа:** Требуется авторизация (любая роль)

**Параметры запроса (Query):**
- `supplier_id` (integer, required) - ID поставщика
- `date_from` (date, optional) - Дата «от» (YYYY-MM-DD)
- `date_to` (date, optional) - Дата «по» (включительно)
...
```json
{
  "receipts": [
    {
      "id": 1,
      "number": "REC-20260203-0001",
      "document_date": "2024-01-15",
      "supplier_id": 1,
      "supplier_document_number": "YT000147365",
      "supplier_document_date": "2024-01-15",
      "status": "posted",
      "created_at": "2024-01-15T12:00:00Z",
      "supplier": { "id": 1, "name": "ООО Запчасти", "inn": "7707123456", "contact": null },
      "lines": [...],
...
```
- `receipts` — список приходных накладных поставщика за период (с полным составом строк и поставщиком)
- `total_count` — количество документов
- `total_amount` — сумма прихода за период (сумма полей `total_amount` по всем накладным)
...
**Описание:** Список приходных накладных

**Права доступа:** Требуется авторизация (любая роль)

**Параметры запроса (Query):**
- `skip`, `limit` — постраничная выборка
- `status` (string, optional) - Фильтр по статусу: `draft`, `posted`

**Ответ:** Массив приходных накладных с полями:
- `id` — внутренний ID документа
- `number` — внутренний номер прихода (например, `REC-20260203-0001`)
- `document_date` — дата оприходования в системе
- `supplier_id` — ID поставщика
- `supplier_document_number` — номер документа поставщика (например, `YT000147365`)
- `supplier_document_date` — дата документа поставщика (если не указана — равна `document_date`)
- `status` — `draft` или `posted`
- `created_at` — дата создания записи
- `supplier` — объект поставщика
- `lines` — массив строк с `part_id`, `quantity`, `purchase_price`, `sale_price`, `part`.

---

### GET `/api/v1/warehouse/receipts/{receipt_id}`
**Описание:** Приходная накладная по ID

**Права доступа:** Требуется авторизация (любая роль)

**Ответ:** Объект накладной с полным составом строк и поставщиком.

---

### POST `/api/v1/warehouse/receipts`
**Описание:** Создание приходной накладной (черновик)
...
```
- Номер накладной генерируется автоматически (формат `REC-YYYYMMDD-NNNN`).
- При проведении (`POST /warehouse/receipts/{id}/post`) по каждой строке увеличивается остаток, создаются транзакции прихода, обновляются цены запчасти (продажная и последняя закупочная).

---

### PUT `/api/v1/warehouse/receipts/{receipt_id}`
**Описание:** Обновление черновика накладной

**Права доступа:** MANAGER, ADMIN

**Возможные ошибки:** Редактировать можно только накладные в статусе `draft`.

---

### POST `/api/v1/warehouse/receipts/{receipt_id}/post`
**Описание:** Проведение приходной накладной (оприходование товара)

**Права доступа:** MANAGER, ADMIN

**Особенности:** Увеличиваются остатки по строкам, создаются транзакции прихода с привязкой к накладной, обновляются продажная и закупочная цены запчастей. Требуется привязанный к пользователю сотрудник.

**Возможные ошибки:**
- `400 Bad Request` - Накладная уже проведена или не содержит строк
- `404 Not Found` - Накладная не найдена
...
**Параметры запроса (Query):** `skip`, `limit`

**Ответ:** Массив объектов с полями `id`, `name`, `inn`, `contact`.

---

### GET `/api/v1/suppliers/{supplier_id}`
**Описание:** Поставщик по ID
...
**Описание:** Обновление поставщика
