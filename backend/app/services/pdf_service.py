...
def _fmt(val: Decimal | float | None) -> str:
    """Форматирование суммы для документов: 1000 → '1 000.00'.

    Значения целые (копеек нет — скидка округляется до рубля в _line_total),
    но в печатных формах показываем их в стандартном бухгалтерском виде с двумя
    знаками после запятой. Значения приходят посчитанными из заказа (источник
    истины — БД).
    """
    if val is None:
        return "0.00"
